...
if ( ! function_exists( '__module_preload' ) ) {
	add_action( 'wp_head', '__module_preload', 11 );
		...
		if ( is_front_page() || is_home() ) {
			...
			if ( false === $critical_css ) {
				...
				if ( is_file( $critical_css_file ) ) {
					...
					set_transient( '_transient_index_critical', $critical_css, HOUR_IN_SECONDS );
				}
			}
...
if ( ! function_exists( '__other_head' ) ) {
	add_action( 'wp_head', '__other_head', 10 );

	/**
	 * @throws JsonException
	 */
	function __other_head(): void {
		...
		if ( is_file( $manifest ) ) {
			...
			if ( isset( $manifest['short_name'] ) ) {
				echo '<meta name="application-name" content="' . $manifest['short_name'] . '">';
				echo '<meta name="apple-mobile-web-app-title" content="' . $manifest['short_name'] . '">';
...
if ( ! function_exists( '__external_fonts' ) ) {
	add_action( 'wp_head', '__external_fonts', 99 );
...
// -----------------------------------------------
// hd/before_header_action
// -----------------------------------------------

if ( ! function_exists( '__skip_to_content_link' ) ) {
	add_action( 'hd/before_header_action', '__skip_to_content_link', 2 );
...
if ( ! function_exists( '__construct_header' ) ) {
	add_action( 'hd/header_action', '__construct_header', 10 );
...
if ( ! function_exists( '_masthead_home_seo_header' ) ) {
	add_action( 'hd/masthead', '_masthead_home_seo_header', 10 );
	...
	function _masthead_home_seo_header(): void {
		...
		echo '<h1 class="sr-only">' . $home_heading . '</h1>';
	}
}
...
if ( ! function_exists( '_masthead_top_header' ) ) {
	add_action( 'hd/masthead', '_masthead_top_header', 12 );
...
if ( ! function_exists( '_masthead_header' ) ) {
	add_action( 'hd/masthead', '_masthead_header', 13 );
...
if ( ! function_exists( '_masthead_bottom_header' ) ) {
	add_action( 'hd/masthead', '_masthead_bottom_header', 14 );
...
if ( ! function_exists( '_masthead_container_open' ) ) {
	add_action( 'hd/masthead', '_masthead_container_open', 11 );
...
if ( ! function_exists( '_masthead_container_close' ) ) {
	add_action( 'hd/masthead', '_masthead_container_close', 99 );
...
if ( ! function_exists( '_masthead_custom' ) ) {
	add_action( 'hd/masthead', '_masthead_custom', 98 );
