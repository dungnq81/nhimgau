...
if ( ! function_exists( '__wp_footer' ) ) {
	...
	function __wp_footer(): void {
		if ( apply_filters( 'hd/back_to_top_filter', true ) ) {
			echo apply_filters(
				'hd/back_to_top_output_filter',
				sprintf(
					...
					absint( apply_filters( 'hd/back_to_top_scroll_speed_filter', 400 ) ),
					absint( apply_filters( 'hd/back_to_top_scroll_start_filter', 300 ) ),
...
if ( ! function_exists( '__construct_footer' ) ) {
	add_action( 'hd/footer_action', '__construct_footer', 10 );
...
if ( ! function_exists( '_footer_widgets' ) ) {
	add_action( 'hd/construct_footer', '_footer_widgets', 10 );
...
if ( ! function_exists( '_footer_credit' ) ) {
	add_action( 'hd/construct_footer', '_footer_credit', 11 );
	...
	function _footer_credit(): void {
        ...
        <div id="footer-credit" class="footer-credit">
			...
			$copyright = sprintf(
				...
				date( 'Y' ),
				get_bloginfo( 'name' ),
				apply_filters( 'hd/copyright_text_filter', __( 'All rights reserved.', TEXT_DOMAIN ) )
			);

			echo apply_filters( 'hd/copyright_filter', $copyright );
...
if ( ! function_exists( '_footer_container_open' ) ) {
	add_action( 'hd/construct_footer', '_footer_container_open', 10 );
...
if ( ! function_exists( '_footer_container_close' ) ) {
	add_action( 'hd/construct_footer', '_footer_container_close', 99 );
...
if ( ! function_exists( '_footer_custom' ) ) {
	add_action( 'hd/construct_footer', '_footer_custom', 98 );
