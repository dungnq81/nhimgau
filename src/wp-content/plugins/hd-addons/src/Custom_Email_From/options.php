...
if ( ! empty( $filter_custom_emails ) ) {
        ...
        <div class="section section-text" id="section_emails_from">
            ...
            <div class="row">
                <div class="controls">
                    ...
                    <input value="<?= $from_name ?>" type="text" class="!w[100%]" name="<?= esc_attr( $key ) ?>_from_name"
                           id="<?= esc_attr( $key ) ?>_from_name">
                </div>
            </div>
            <div class="row">
                <div class="controls">
                    <label for="<?= esc_attr( $key ) ?>_from_email"><?php _e( 'From Email', ADDONS_TEXT_DOMAIN ); ?></label>
                    <input value="<?= $from_email ?>" type="email" class="!w[100%]" name="<?= esc_attr( $key ) ?>_from_email"
                           id="<?= esc_attr( $key ) ?>_from_email" placeholder="e.g. &quot;abc@gmail.com&quot;"
                           aria-describedby="<?= esc_attr( $key ) ?>_from_label">
                </div>
	...
	echo '<p style="color:#d63638">' . __( 'Chưa được khởi tạo', ADDONS_TEXT_DOMAIN ) . '</p>';
}
