...
<div class="section section-radio" id="section_svg">
    ...
    <div
        class="desc"><?php _e( '<b>Security notice:</b> Every SVG image is an XML file that can contain <b>malicious code</b>, potentially leading to <b>XSS</b> or <b>injection attacks</b>.', ADDONS_TEXT_DOMAIN ); ?></div>
    <div class="option inline-option">
        <div class="controls">
            <div class="inline-group">
				<?php foreach ( $options as $key => $opt ) { ?>
                    <label>
                        <input type="radio" name="svgs" class="radio" id="svgs-<?= $key ?>"
                               value="<?= $key ?>" <?php echo checked( $svgs, $key ); ?> />
                        <span><?= $opt ?></span>
                    </label>
				<?php } ?>
            </div>
