...
if ( $login_security_default['enable_custom_login_options'] ) {
    ...
    <div class="section section-text" id="section_custom_login_uri">
        ...
        <div
            class="desc"><?php _e( 'Attackers frequently target <b>/wp-admin</b> or <b>/wp-login.php</b> as the default login URL for WordPress. Changing it can help prevent these attacks and provide a more memorable login URL.', ADDONS_TEXT_DOMAIN ); ?></div>
        <div class="option">
            <div class="controls control-prefix" style="height: unset;">
                <div class="prefix">
                    <span class="input-txt"
                          title="<?= esc_attr( esc_url( network_home_url( '/' ) ) ) ?>"><?= esc_url( network_home_url( '/' ) ) ?></span>
                </div>
				<?php
				...
				?>
                <input value="<?php echo esc_attr( $custom_login_uri ); ?>" class="input" type="text" id="custom_login_uri"
                       name="custom_login_uri" placeholder="<?= esc_attr( $custom_login_uri ) ?>">
            </div>
        </div>
    </div>
<?php } ?>
...
<div class="section section-select" id="section_login_ips_access">
    ...
    <div
        class="desc"><?php _e( 'By default, your WordPress login page is accessible from any IP address. You can use this feature to restrict login access to specific IPs or ranges of IPs to prevent brute-force attacks or malicious login attempts.<br><b>Ex:</b> 192.168.0.1, 192.168.0.1-100, 192.168.0.1/4', ADDONS_TEXT_DOMAIN ); ?></div>
    <div class="option">
        <div class="controls">
            <div class="select_wrapper">
                <select multiple placeholder="Enter IP addresses" class="select select2-ips !w[100%]" name="login_ips_access"
                        id="login_ips_access">
					<?php
					if ( $login_ips_access ) {
...
<div class="section section-select" id="section_disable_ips_access">
    ...
    <div
        class="desc"><?php _e( 'List of IP addresses or ranges of IPs blocked from accessing the login page.<br><b>Ex:</b> 192.168.0.1, 192.168.0.1-100, 192.168.0.1/4', ADDONS_TEXT_DOMAIN ); ?></div>
    <div class="option">
        <div class="controls">
            <div class="select_wrapper">
                <select multiple placeholder="Enter IP addresses" class="select select2-ips !w[100%]" name="disable_ips_access"
                        id="disable_ips_access">
					<?php
...
<div class="section section-checkbox !hidden" id="section_two_factor_authentication">
    <label class="heading"
           for="two_factor_authentication"><?php _e( 'Two-factor Authentication for Admin & Editors Users', ADDONS_TEXT_DOMAIN ); ?></label>
    <div
        class="desc"><?php _e( 'Two-factor authentication forces admin users to login only after providing a token, generated from the Google Authenticator application. When you enable this option, all admin & editor users will be asked to configure their two-factor authentication in the Authenticator app on their next login.', ADDONS_TEXT_DOMAIN ) ?></div>
    <div class="option">
        <div class="controls">
            <input disabled type="checkbox" class="checkbox" name="two_factor_authentication"
                   id="two_factor_authentication" <?php echo checked( $two_factor_authentication, 1 ); ?> value="1">
        </div>
